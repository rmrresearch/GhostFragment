...
// ``simde::type::fragmented_molecule_object`` to a
// ``ghostfragment::type::nmers``. This has two major steps:

// #. Cap the input fragments
// #. Screen unions of the capped fragments

// Since it makes taking unions of fragments easier, the input fragments have
// not been capped. However, for most screening methods caps are needed. Thus
// the first step is to cap the fragments. These caps will not be on the
// returned NMers, the caps are only used internally for the screening process.
// Once we have capped the fragments, we form the nmers, and finally return the
// nmers which survive the screening process.
// )";
...
//     const auto n        = inputs.at("N-Mer Truncation
//     Order").value<unsigned>();
