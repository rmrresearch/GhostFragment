...
TEST_CASE("NMerSystem") {
    ...
    auto water4                  = fragmented_water(4);
    ...
    SECTION("n_electrons(frag)") {
        REQUIRE_THROWS_AS(defaulted.n_electrons(water3[0]), std::runtime_error);
        REQUIRE(has_value.n_electrons(water3[0]) == 10);
        ...
        REQUIRE_THROWS_AS(has_value.n_electrons(water4[0]), std::out_of_range);
    }
